...
class FinancialMetric(BaseModel):
    metric_name: str = Field(
        description=(
            "Name of financial metric e.g. Consolidated Revenue, "
    ...
    current_period: str = Field(
        description=(
            "Current period value e.g. $1.2B, $180M, $450M"
        )
    )

    previous_period: str = Field(
        description=(
            "Comparison period value e.g. $1.04B Q2 2025, "
            "$195M Q2 2025"
        )
    )
...
class AegisFinancialReport(BaseModel):
    company_name: str = Field(
        description="Organization name e.g. TechCorp"
    )

    period: str = Field(
        description="Reporting Period e.g. Q2 2026"
    )

    executive_summary: str = Field(
        description=(
            "Concise performance, capital allocation, "
            "and risk summary for buy-side teams"
        )
    )

    key_metrics: List[FinancialMetric] = Field(
        description=(
            "Extracted financial metrics including YoY Revenue, "
    ...
    key_risk_factors: List[str] = Field(
        description=(
            "Flagged guidance revisions, supply chain bottlenecks, "
            "or regulatory risks"
        )
    )
    ...
    growth_drivers: List[str] = Field(
        description=(
            "Primary business or revenue expansion drivers"
        )
    )
...
RAW_SEC_FILINGS = [
    ...
    to $180M (down from $195M in Q2 2025), driven by increased
    infrastructure capital expenditures in AI data center deployment.

    Total outstanding debt stands at $600M offset by $150M
    in cash equivalents, resulting in a Net Debt position of
    $450M (compared to $400M in Q2 2025).
    """,
    ...
    in semiconductor hardware delivery and pending regulatory
    approval delays in cross-border AI data transfers.
    """
]
...
def setup_rag_engine():
    ...
    cleaning, text splitting, embeddings,
    and Chroma vector store initialization.
    """
...
Analyze the provided SEC disclosures, balance sheets,
and earnings call context to evaluate financial risk.
...
Deliver an accurate, structured due diligence report
adhering strictly to the facts provided above.

Do not invent any numbers, financial metrics,
companies, dates, or facts outside the provided context.

If information is not available in the retrieved context,
do not fabricate it.
...
if st.button(
    ...
    if not groq_api_key:
        ...
        try:
            ...
            with st.spinner(
                "Processing SEC disclosures via Groq LLaMA 3.3 70B..."
            ):
